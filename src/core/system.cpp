...
bool SwitchMediaSubImage(u32 index)
{
  ...
  {
    g_host_interface->AddFormattedOSDMessage(
      10.0f, g_host_interface->TranslateString("OSDMessage", "Failed to switch to subimage %u in '%s': %s."), index,
      image->GetFileName().c_str(), error.GetCodeAndMessage().GetCharArray());
    g_cdrom.InsertMedia(std::move(image));
    return false;
  }
  ...
  g_host_interface->AddFormattedOSDMessage(
    ...
    image->GetSubImageMetadata(index, "title").c_str(), index, image->GetMetadata("title").c_str());
  g_cdrom.InsertMedia(std::move(image));
