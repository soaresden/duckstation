...
bool DoState(StateWrapper& sw)
{
  ...
  sw.DoEx(&ram_size, 48, static_cast<u32>(RAM_2MB_SIZE));
  if (ram_size != g_ram_size)
  {
    ...
    UpdateFastmemViews(m_fastmem_mode, CPU::g_state.cop0_regs.sr.Isc);
  }
...
void UpdateFastmemViews(CPUFastmemMode mode, bool isolate_cache)
{
  ...
  if (mode == CPUFastmemMode::MMap)
  {
    ...
    auto MapRAM = [](u32 base_address, bool writable) {
      ...
      auto view = m_memory_arena.CreateView(MEMORY_ARENA_RAM_OFFSET, g_ram_size, writable, false, map_address);
      if (!view)
      {
...
u8* GetMemoryRegionPointer(MemoryRegion region)
{
  switch (region)
  {
    ...
    case MemoryRegion::RAMMirror2:
      return (g_ram + ((RAM_2MB_SIZE * 4) & g_ram_mask));

    case MemoryRegion::RAMMirror3:
      return (g_ram + ((RAM_8MB_SIZE * 6) & g_ram_mask));
